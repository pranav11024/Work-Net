              ...
              <table>
                <thead>
                  <tr class="row100 head">
                    ...
                    <th class="cell100 column6">Contact</th>
...
while ($row = mysqli_fetch_array($result)) {
    ...
    for ($x = 0; $x < ($count - 1); $x++) {
        ...
        // Fetch employer details from the job table
        $sqlJobDetails = "SELECT employer, title FROM job WHERE job_id = '$r[0]' ";
        $resultJobDetails = mysqli_query($conn, $sqlJobDetails);

        while ($rowJobDetails = mysqli_fetch_array($resultJobDetails)) {
            $company = $rowJobDetails['employer'];
            $designation = $rowJobDetails['title'];

            // Fetch employer email from the employer table
            

$sqlEmployerEmail = "SELECT email, contact FROM employer WHERE name = '$company' ";
$resultEmployerInfo = mysqli_query($conn, $sqlEmployerEmail);
$rowEmployerInfo = mysqli_fetch_array($resultEmployerInfo);
$employerEmail = $rowEmployerInfo['email'];
$employerContact = $rowEmployerInfo['contact'];

echo '<td class="cell100 column2">' . $designation . '</td>';
echo '<td class="cell100 column3">' . $company . '</td>';
//echo '<td class="cell100 column4">' . $r[1] . '</td>';
echo '<td class="cell100 column5">' . $employerEmail . '</td>';
echo '<td class="cell100 column6">' . $employerContact . '</td>'; // Include employer contact
